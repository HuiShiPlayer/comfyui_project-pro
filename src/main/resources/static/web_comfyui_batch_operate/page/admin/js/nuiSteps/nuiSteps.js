...
(function($){
    $.fn.step = function(options) {
        ...
        var barWidth=opts.initStep<size?100/(size)+100*(opts.initStep-1)/size : 100;
        ...
        this.goStep=function(page) {
            ...
            this.find(".step-header li").each(function (i, li) {
                if (i>=page){
                    $li=$(li);
                    $li.prev("li").removeClass("step-active-prev").find("span").removeClass("icon-checkmark");
            ...
            barWidth=page<size?100/(size)+100*(page-1)/size : 100;
            this.find(".step-bar-active").animate({
                    "width": barWidth+"%"},
                opts.speed, function() {
                    $(this).parent(".step-bar").next("ul").find("li").each(function (i, li) {
                        $li=$(li);
                        if (i<page){
                            $li.addClass('step-active');
                            $li.prev("li").addClass("step-active-prev").find("span").addClass("icon-checkmark");
                            if(opts.scrollTop){
                                $('html,body').animate({scrollTop:0}, 'slow');
                            }
                        }
