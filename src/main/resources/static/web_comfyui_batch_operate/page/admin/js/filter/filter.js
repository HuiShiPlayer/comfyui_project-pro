    ...
    $.fn.comboboxfilter.methods = {
        ...
        setData: function (target, options, data, targetContain) {
            ...
            $.each(data, function (i, item) {
                var listnode = $(' <li></li>');
                var clicka = $('<a class="filter-tag" href="javascript:;" data-value="' + item[options.idField] + '" data-text="' + item[options.textField] + '"><i></i>' + item[options.textField] + '</a>').data('data', item);
        ...
        addSelected: function (pointTarget, target, itemData, options, targetContain) {
          ...
          var anode = $('<a href="javascript:;">' + itemData[options.textField] + '</a>');
                //创建X ,点击则移除选择项
                var inode = $('<i class="" data-type="ParentCatelog" data-value="' + itemData[options.idField] + '">&times;</i>').unbind('click').bind('click', function (e) {
                ...
                $(target).unbind('click.selected-tag').bind('click.selected-tag', function (e) {
                    ...
                    /*2019-01-07 新增代码 ↓*/
                   if(flag_text_chosen){
	                   var tid = $(target).attr('data-value');
