...
var main = new Vue({
	...
	methods: {
		getFrames(type,item){
			...
			console.log(app.base_path_contact_system+ "/getFrame/"+ type+"/"+item.count+"/"+item.name)
			 var xhr = new XMLHttpRequest();
			 xhr.open("GET",app.base_path_contact_system+ "/getFrame/"+ type+"/"+item.count+"/"+item.name, true);//true 为异步
			 ...
			 xhr.onreadystatechange = function () {
				 var fileName = null;
				 if(type == 1){
					 fileName = "startFrame_" + item.name.substring(0,item.name.length-4)+".png";
				 }else if(type == 2){
					 fileName = "endFrame_" + item.name.substring(0,item.name.length-4)+".png";
				 }else if(type == 4){
					 fileName = item.count+"_Frame_" + item.name.substring(0,item.name.length-4)+".png";
				 }
				  ...
				  if(xhr.readyState == 4 &&xhr.status == 401){
					  alert_warning("参数错误")
				  	  ...
				  	  alert_warning("跳帧设置错误")
				  }
				  else  if(xhr.readyState == 4 &&xhr.status == 410){
				  	  alert_warning("跳帧设置过大")
				  }
