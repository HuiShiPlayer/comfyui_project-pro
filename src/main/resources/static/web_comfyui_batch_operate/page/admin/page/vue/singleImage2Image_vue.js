...
var main = new Vue({
	...
	mounted() {		
		var data = localStorage.getItem("singleImage2Image")
		if (data) {
		...
		var singleInitData = localStorage.getItem("singleInitData")
		if (singleInitData) {
			singleInitData = JSON.parse(singleInitData)
			this.width = singleInitData.width
			this.height = singleInitData.height
			/* task5.1 */
			for (var i = 0; i < singleInitData.list.length; i++) {
				if (singleInitData.list[i]) {
					var objectData = {
						prompt: "",
						pre_image: "img/bg_image_1.png",
						image_width: 100,
						image_height: 100,
						referenceImage: singleInitData.list[i],
						prompVisiable: false,
						isContinue: false
					}
					// 对每个元素执行操作
			...
			localStorage.removeItem("singleInitData")
		}
	...
	methods: {
				...
				success: function(res) {
					...
					if(res.status == 200){
						if(res.data.resources && res.data.resources.length>0){
							that.globalReference = app.base_path_contact_system.split("admin")[0]+res.data.resources[0]
		...
		/* task5.1 */
		checkTask() {
			console.log("checkTask I2I")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskListi2i");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskListi2i", resTaskListStr)
									}
		...
		nextStep(stepNum) {
			...
			if (stepNum == 4) {
				...
				console.log(url)
				if (url.includes("singleImage2Image")) {
					location.reload()
				} else {
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskListi2i');
			localStorage.removeItem("singleImage2Image")
			location.reload()
		},
		/* task5.1 */
		saveDataInCache() {
			...
			localStorage.setItem("singleImage2Image", JSON.stringify(data))
		},
		saveCapcut() {
			var that = this
			...
			$.ajax({
				...
				success: function(response) {
					if (response.status == 200) {
						...
						throw new Error(`HTTP错误: ${response.msg}`); 
		...
		async generateImage(data, isbatch, taskIndex) {
			...
			try {
				...
				var Imagename = data.referenceImage.split('/').pop().split('?')[0];
				/* 图片名字 */
				...
				if (that.defaultSource == 1) {
					...
					if (uploadTocomfyUIImgs[data.referenceImage]) {
						name = uploadTocomfyUIImgs[data.referenceImage]
					} else {
						name = await that.uploadImageToComfyUI(data.referenceImage, Imagename);
						uploadTocomfyUIImgs[data.referenceImage] = name
					}
				}
				...
				if (that.defaultSource == 1) {
					response = await fetch(app.base_path_contact_system + "/comfyui/rhprompt", {
						...
						body: JSON.stringify({
							"nodeInfoList": that.generateWorkFlow(itemPrompt, s, that
								.totalSteps, w, h, name, data.param1, data.param2, data
					...
					response = await fetch(app.base_path_contact_system + "/comfyui/prompt", {
						...
						body: JSON.stringify({
							'prompt': that.generateWorkFlow(itemPrompt, s, that.totalSteps,
								w,
								h, name, data.param1, data.param2, data.param3, data
								.param4, data.param5),
							"taskIndex": taskIndex,
							"outputsNum": that.outputsNum
						}),
		...
		generateWorkFlow(prompt, seed, totalSteps, width, height, name, param1, param2, param3, param4,
			param5) {
			// var newapi = api.replace(/\\/g, "\\\\");
			var that = this

			var jsondata = that.apiFunction(prompt, seed, totalSteps, width, height, name, param1, param2,
						...
						success: function(res) {
							if (res.status == 200) {
								var objectData = {
									...
									image_width: 100,
									image_height: 100,
									referenceImage: app.base_path_contact_system.split("admin")[
										0] + res.data,
									prompVisiable: false,
									isContinue: false
								}
				...
				if (item == 2) {
					...
					if (lenghtTempleteData <= lenghtData) {
						for (var i = 0; i < lenghtTempleteData; i++) {
							// await that.convertImageToBase64WithCanvas(responseData.data[i].url).then(item => {										
							// 	that.list[i].referenceImage=item
							// })
