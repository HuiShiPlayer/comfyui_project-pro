...
var main = new Vue({
	...
	mounted() {
		/* task5.1 */
		console.log("重载")
		var data = localStorage.getItem("generateImage")
		if (data) {
	...
	methods: {
		...
		/* task5.1 */
		clearCache() {
			var resTaskListStr = localStorage.getItem("resTaskListGenrate");
			...
			localStorage.removeItem('resTaskListGenrate');
		...
		saveDataInCache() {
			...
			localStorage.setItem("generateImage", JSON.stringify(data))
		},
		saveCapcut() {
			var that = this
		...
		async cancelTask() {
			...
			// const response =await fetch(app.base_path_contact_comfyui + "/interrupt", {
			// 					method: 'post'
			// 				});
		...
		checkTask() {
			...
			console.log("checkTask generateImage")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskListGenrate");
			if (!app.isBlank(resTaskListStr) && JSON.parse(resTaskListStr).length != 0) {
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskListGenrate", resTaskListStr)
									}
		...
		generateWorkFlow(data) {
			...
			if (!app.isBlank(this.globalPrompt2)) {
				prompt = this.globalPrompt2.concat(",", prompt);
			}
			if (!app.isBlank(this.globalPrompt1)) {
				prompt = this.globalPrompt1.concat(",", prompt);
			}
			...
			var jsondata = that.apiFunction(prompt, s, that.totalSteps, w, h, data.param1, data.param2, data
