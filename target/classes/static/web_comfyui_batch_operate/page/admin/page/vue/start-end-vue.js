...
var main = new Vue({
	...
	mounted() {
		var data = localStorage.getItem("start-end")
		if (data) {
		...
		var start2endInitData = localStorage.getItem("start2endInitData")
		if (start2endInitData) {
			start2endInitData = JSON.parse(start2endInitData)
			this.width = start2endInitData.width
			this.height = start2endInitData.height

			for (var i = 0; i < start2endInitData.list.length - 1; i++) {
				if (start2endInitData.list[i]) {
					var objectData = {
						prompt: "",
						pre_image: "img/bg_image_1.png",
						image_width: 100,
						image_height: 100,
						pre_video: null,
						referenceImage1: start2endInitData.list[i],
						referenceImage2: start2endInitData.list[i + 1],
						prompVisiable: false,
						referenceAudio1: null,
						referenceAudio2: null,
						referenceAudio3: null,
			...
			localStorage.removeItem("start2endInitData")
		}
	...
	methods: {
		...
		/* task5.1 */
		checkTask() {
			console.log("checkTask fl2v")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskListfl2v");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskListfl2v", resTaskListStr)
									}
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskListfl2v');
		...
		saveDataInCache() {
			...
			localStorage.setItem("start-end", JSON.stringify(data))
		},
			...
			for (var i = 0; i < e.target.files.length; i++) {
				...
				if (file) {
					...
					$.ajax({
						...
						success: function(res) {
							if (res.status == 200) {
								imagesList.push(app.base_path_contact_system.split("admin")[0] + res
									.data)
							}
						},
						error: function(jqXHR, textStatus, errorThrown) {
			...
			if (imagesList.length == 1) {
				...
				for (var i = 1; i < imagesList.length; i++) {
					var objectData = {
						...
						image_width: 100,
						image_height: 100,
						referenceImage1: imagesList[i - 1],
						referenceImage2: imagesList[i],
						referenceAudio1: null,
						referenceAudio2: null,
						referenceAudio3: null,
						prompVisiable: false,
		...
		async batchGenerate() {
			...
			for (var i = 0; i < this.list.length; i++) {
				console.log(`this.list[i].isContinue:${this.list[i].isContinue}`)
		...
		async generateImage(data, isbatch, taskIndex) {
			...
			try {
				...
				if (!data.referenceImage1 || !data.referenceImage2) {
					setTimeout(function() {
				...
				if (that.defaultSource == 1) {
					response = await fetch(app.base_path_contact_system + "/comfyui/rhprompt", {
						...
						body: JSON.stringify({
							"nodeInfoList": that.generateWorkFlow(itemPrompt, name1, name2,
								audio1, audio2, audio3, s, that
								.totalSteps, w, h, l, that.blocks_to_swap, data.param1,
								data
					...
					response = await fetch(app.base_path_contact_system + "/comfyui/prompt", {
						...
						body: JSON.stringify({
							prompt: that.generateWorkFlow(itemPrompt, name1, name2, audio1,
								audio2, audio3, s, that
								.totalSteps, w, h, l, that.blocks_to_swap, data.param1,
								data
								.param2, data.param3, data.param4, data.param5),
							"taskIndex": taskIndex,
		...
		async changeCulumDataByTemplete(item) {
			...
			if (responseData.status == 200) {
				if (item == 0) {
					...
					for (var i = 0; i < responseData.data.length - 1; i++) {
						...
						var data = {
							...
							referenceImage2: responseData.data[i + 1].url,
							referenceAudio1: null,
							referenceAudio2: null,
							referenceAudio3: null,
							prompVisiable: false,
				...
				if (item == 2) {
					var lenghtTempleteData = responseData.data.length - 1
					...
					if (lenghtTempleteData <= lenghtData) {
						for (var i = 0; i < lenghtTempleteData; i++) {
							...
							that.list[i].referenceImage2 = responseData.data[i + 1].url
		...
		generateWorkFlow(prompt, name1, name2, audio1, audio2, audio3, seed, totalSteps, width, height, length,
			blocks_to_swap, param1,
			param2, param3, param4, param5) {
			// var newapi = api.replace(/\\/g, "\\\\");

			var that = this
			var jsondata = that.apiFunction(prompt, name1, name2, audio1, audio2, audio3, seed, totalSteps,
				width, height, length,
