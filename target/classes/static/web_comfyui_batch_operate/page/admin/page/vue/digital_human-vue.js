...
var main = new Vue({
	...
	mounted() {
		var data = localStorage.getItem("digital_human")
		if (data) {
		...
		var digitalHumanInitData = localStorage.getItem("digitalHumanInitData")
		if (digitalHumanInitData) {
			digitalHumanInitData = JSON.parse(digitalHumanInitData)
			this.width = digitalHumanInitData.width
			this.height = digitalHumanInitData.height
			for (var i = 0; i < digitalHumanInitData.list.length; i++) {
				if (digitalHumanInitData.list[i]) {
					var objectData = {
						prompt: "",
						pre_image: "img/bg_image_1.png",
						image_width: 100,
						image_height: 100,
						referenceImage: digitalHumanInitData.list[i],
						referenceVideo: null,
						referenceAudio1: null,
						referenceAudio2: null,
						prompVisiable: false,
			...
			localStorage.removeItem("digitalHumanInitData")
		}
		var baseVideos = localStorage.getItem("baseVideos")
		if (baseVideos) {
			baseVideos = JSON.parse(baseVideos)
			this.width = baseVideos.width
			this.height = baseVideos.height
			for (var i = 0; i < baseVideos.list.length; i++) {
				if (baseVideos.list[i]) {
					var objectData = {
						prompt: "",
						pre_image: "img/bg_image_1.png",
						image_width: 100,
						image_height: 100,
						referenceImage: null,
						referenceVideo: baseVideos.list[i],
						referenceAudio1: null,
						referenceAudio2: null,
						prompVisiable: false,
						isContinue: false
			...
			localStorage.removeItem("baseVideos")
		}
	...
	methods: {
		...
		checkTask() {
			console.log("checkTask vedioEdit")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskListedit");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskListedit", resTaskListStr)
									}
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskListedit');
		...
		saveDataInCache() {
			...
			localStorage.setItem("digital_human", JSON.stringify(data))
		},
		saveCapcut() {
			var that = this
		...
		async generateImage(data, isbatch, taskIndex) {
			...
			try {
				...
				if (!app.isBlank(data.referenceVideo)) {
					Videoname = data.referenceVideo.split('/').pop().split('?')[0];
				...
				var Sk1 = null;
				var Sk2 = null;
				var maskP1 = data.maskP1;
				var maskP2 = data.maskP2;
				if (data.length1) {
					l1 = data.length1
				} else {
					l1 = that.length
					...
					response = await fetch(app.base_path_contact_system + "/comfyui/rhprompt", {
						...
						body: JSON.stringify({
							"nodeInfoList": that.generateWorkFlow(itemPrompt, name, video,
								s, that
								.totalSteps, w, h, l1, Sk1, maskP1, l2, Sk2, maskP2,
								that
								.blocks_to_swap, Audio1Name, Audio2Name, data.param1,
								data
				...
				if (item == 2) {
					...
					if (lenghtTempleteData <= lenghtData) {
						for (var i = 0; i < lenghtTempleteData; i++) {
							...
							if (!that.list[i].referenceAudio1 && that.globalReferenceAudio1 && that
								.globalReferenceAudio1 != null && that.globalReferenceAudio1 != 'null') {
								that.list[i].referenceAudio1 = that.globalReferenceAudio1
							}
							if (!that.list[i].referenceAudio2 && that.globalReferenceAudio2 && that
								.globalReferenceAudio2 != null && that.globalReferenceAudio2 != 'null') {
								that.list[i].referenceAudio2 = that.globalReferenceAudio2
							}
						}
					} else {
		...
		generateWorkFlow(prompt, name, video, seed, totalSteps, width, height, length1, skip1, maskP1, length2,
			skip2, maskP2, blocks_to_swap, referenceAudio1, referenceAudio2, param1, param2, param3, param4,
			param5) {
			// var newapi = api.replace(/\\/g, "\\\\");
			var that = this
			var jsondata = that.apiFunction(prompt, name, video, seed, totalSteps, width, height, length1,
				skip1, maskP1, length2, skip2, maskP2, blocks_to_swap, referenceAudio1, referenceAudio2,
