...
var main = new Vue({
	...
	beforeDestroy() {
		...
		console.log("离开混合生视频页面")
	},
	mounted() {
		var data = localStorage.getItem("mulImgs2Vedio")
		if (data) {
		...
		var mulReference2VideoInitData = localStorage.getItem("mulReference2VideoInitData")
		if (mulReference2VideoInitData) {
			mulReference2VideoInitData = JSON.parse(mulReference2VideoInitData)
			this.width = mulReference2VideoInitData.width
			this.height = mulReference2VideoInitData.height

			for (var i = 0; i < mulReference2VideoInitData.list.length - 1; i++) {
				if (mulReference2VideoInitData.list[i]) {
					var objectData = {
						prompt: "",
						pre_image: "img/bg_image_1.png",
						image_width: 100,
						image_height: 100,
						pre_video: null,
						referenceImage1: mulReference2VideoInitData.list[i],
						referenceImage2: null,
						referenceImage3: null,
						referenceAudio1: null,
						referenceAudio2: null,
			...
			localStorage.removeItem("mulReference2VideoInitData")
		}
	...
	methods: {
		...
		checkTask() {
			console.log("checkTask Mul2v")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskListmul2v");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskListmul2v", resTaskListStr)
									}
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskListmul2v');
			localStorage.removeItem("mulImgs2Vedio")
			location.reload()
		},
		saveDataInCache() {
			console.log('数据缓存')
			...
			localStorage.setItem("mulImgs2Vedio", JSON.stringify(data))
		},
		...
		async generateImage(data, isbatch, taskIndex) {
			...
			try {
				...
				if (imageCount == 0) {
					console.log("调用文生视频 API == Type0")
					that.getDefualtApi(1)
					if (!that.api) {
						setTimeout(function() {
							alert_warning("未设文生视频API")
							$("#cancel-progress").click()
						}, 1000)
						return
					} else {
					...
					console.log("调用图生视频 API == Type4")
					that.getDefualtApi(2)
					if (!that.api) {
						setTimeout(function() {
							alert_warning("未设置图生视频API")
							$("#cancel-progress").click()
						}, 1000)
						return
					} else {
					...
					that.getDefualtApi(3)
					console.log("调用双图|首尾帧 API == Type5")
					if (!that.api) {
						setTimeout(function() {
							alert_warning("未设置双图|首尾帧API")
							$("#cancel-progress").click()
						}, 1000)
						return
					} else {
					...
					that.getDefualtApi(10)
					console.log("调用三图参考 API == Type6")
					if (!that.api) {
						setTimeout(function() {
							alert_warning("未设置三图参考API")
							$("#cancel-progress").click()
						}, 1000)
				...
				if (imageCount == 0) {
					apiFunctionRes = that.generateWorkFlow0(itemPrompt, s, that.totalSteps, w, h, l, that
						.blocks_to_swap, data.param1, data.param2, data.param3, data.param4, data.param5
					)
				} else if (imageCount == 1) {
					if (name1) {
						console.log(name1)
						apiFunctionRes = that.generateWorkFlow1(itemPrompt, name1, audio1, audio2, audio3,
							s, that
							.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2, data
							.param3, data.param4, data.param5)
					} else if (name2) {
						apiFunctionRes = that.generateWorkFlow1(itemPrompt, name2, audio1, audio2, audio3,
							s, that
							.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2, data
							.param3, data.param4, data.param5)
					} else if (name3) {
						apiFunctionRes = that.generateWorkFlow1(itemPrompt, name3, audio1, audio2, audio3,
							s, that
							.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2, data
							.param3, data.param4, data.param5)
					...
					if (!name1) {
						apiFunctionRes = that.generateWorkFlow2(itemPrompt, name2, name3, audio1, audio2,
							audio3, s,
							that.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2,
							data.param3, data.param4, data.param5)
					} else if (!name2) {
						apiFunctionRes = that.generateWorkFlow2(itemPrompt, name1, name3, audio1, audio2,
							audio3, s,
							that.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2,
							data.param3, data.param4, data.param5)
					} else if (!name3) {
						apiFunctionRes = that.generateWorkFlow2(itemPrompt, name1, name2, audio1, audio2,
							audio3, s,
							that.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2,
							data.param3, data.param4, data.param5)
					}
				} else if (imageCount == 3) {
					apiFunctionRes = that.generateWorkFlow3(itemPrompt, name1, name2, name3, audio1, audio2,
						audio3,
						s, that.totalSteps, w, h, l, that.blocks_to_swap, data.param1, data.param2, data
						.param3, data.param4, data.param5)
				}
		...
		generateWorkFlow0(prompt, seed, totalSteps, width, height, length, blocks_to_swap, param1, param2,
			param3, param4, param5) {
			var that = this
			var jsondata = that.apiFunction0(prompt, seed, totalSteps, width, height, length, blocks_to_swap,
				param1, param2, param3, param4, param5)

			return jsondata
		},
		generateWorkFlow1(prompt, name1, audio1, audio2, audio3, seed, totalSteps, width, height, length,
			blocks_to_swap, param1, param2, param3, param4, param5) {
			// var newapi = api.replace(/\\/g, "\\\\");

			var that = this
			var jsondata = that.apiFunction1(prompt, name1, audio1, audio2, audio3, seed, totalSteps, width,
				height,
				length, blocks_to_swap, param1, param2, param3, param4, param5)

			return jsondata
		},
		generateWorkFlow2(prompt, name1, name2, audio1, audio2, audio3, seed, totalSteps, width, height, length,
			blocks_to_swap, param1, param2, param3, param4, param5) {
			// var newapi = api.replace(/\\/g, "\\\\");

			var that = this
			var jsondata = that.apiFunction2(prompt, name1, name2, audio1, audio2, audio3, seed, totalSteps,
				width,
				height, length, blocks_to_swap, param1, param2, param3, param4, param5)

			return jsondata
		},
		generateWorkFlow3(prompt, name1, name2, name3, audio1, audio2, audio3, seed, totalSteps, width, height,
			length,
			blocks_to_swap, param1, param2, param3, param4, param5) {
			// var newapi = api.replace(/\\/g, "\\\\");

			var that = this
			var jsondata = that.apiFunction3(prompt, name1, name2, name3, audio1, audio2, audio3, seed,
				totalSteps,
