...
var main = new Vue({
	...
	mounted() {
		var data = localStorage.getItem("threeImages2Image")
		if (data) {
	...
	methods: {
		...
		/* task5.1 */
		checkTask() {
			console.log("checkTask 3I2I")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskList3i2i");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskList3i2i", resTaskListStr)
									}
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskList3i2i');
			localStorage.removeItem("threeImages2Image")
			location.reload()
		},
		saveDataInCache() {
			console.log('数据缓存')
			...
			localStorage.setItem("threeImages2Image", JSON.stringify(data))
		},
		saveCapcut() {
			var that = this
					...
					response = await fetch(app.base_path_contact_system + "/comfyui/rhprompt", {
						...
						body: JSON.stringify({
							"nodeInfoList": that.generateWorkFlow(itemPrompt, s, that
								.totalSteps, w, h,
								name1, name2, name3, data.param1, data.param2, data
								.param3,
		...
		generateWorkFlow(prompt, seed, totalSteps, width, height, name1, name2, name3, param1, param2, param3,
			param4, param5) { //todo v2
			// var newapi = api.replace(/\\/g, "\\\\");
			var that = this

			var jsondata = that.apiFunction(prompt, seed, totalSteps, width, height, name1, name2, name3,
			...
			$.ajax({
				...
				data: {
					...
					reference: JSON.stringify({
						...
						referenceImage3: data.referenceImage3,
					})
				},
