...
var main = new Vue({
	...
	mounted() {
		var data = localStorage.getItem("i2v")
		if (data) {
		...
		var i2vInitData = localStorage.getItem("i2vInitData")
		if (i2vInitData) {
			i2vInitData = JSON.parse(i2vInitData)
			this.width = i2vInitData.width
			this.height = i2vInitData.height
			for (var i = 0; i < i2vInitData.list.length; i++) {
				if (i2vInitData.list[i]) {
					var objectData = {
						prompt: "",
						pre_image: "img/bg_image_1.png",
						image_width: 100,
						image_height: 100,
						referenceImage1: i2vInitData.list[i],
						prompVisiable: false,
						pre_video: null,
						referenceAudio1: null,
						referenceAudio2: null,
			...
			localStorage.removeItem("i2vInitData")
		}
		/* rh */
	...
	methods: {
			...
			for(var i =0 ;i<that.list.length;i++){
				...
				$.ajax({
					...
					success: function(res) {
						...
						if(res.data.resources && res.data.resources.length>0){
							that.list[index].referenceImage1 = app.base_path_contact_system.split("admin")[0]+res.data.resources[0]
						}						
					}
		...
		checkTask() {
			console.log("checkTask i2v")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskListi2v");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskListi2v", resTaskListStr)
									}
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskListi2v');
		...
		saveDataInCache() {
			...
			localStorage.setItem("i2v", JSON.stringify(data))
		},
		saveFirtAndEnds() {
			var that = this
		...
		async generateImage(data, isbatch, taskIndex) {
			...
			try {
				...
				var Imagename = data.referenceImage1.split('/').pop().split('?')[0];
				...
				if (that.defaultSource == 1) {
					...
					name = await that.uploadImageToComfyUI(data.referenceImage1, Imagename);
				}
				...
				if (that.defaultSource == 1) {
					response = await fetch(app.base_path_contact_system + "/comfyui/rhprompt", {
						...
						body: JSON.stringify({
							"nodeInfoList": that.generateWorkFlow(itemPrompt, name, audio1,
								audio2, audio3, s, that.totalSteps, w, h, l, that
					...
					response = await fetch(app.base_path_contact_system + "/comfyui/prompt", {
						...
						body: JSON.stringify({
							prompt: that.generateWorkFlow(itemPrompt, name, audio1, audio2,
								audio3,
								s, that.totalSteps, w, h, l, that.blocks_to_swap, data
								.param1, data.param2, data.param3, data.param4, data
								.param5),
							"taskIndex": taskIndex,
							"outputsNum": that.outputsNum
						}),
				...
				if (item == 2) {
					...
					if (lenghtTempleteData <= lenghtData) {
						...
						for (var i = count; i < lenghtTempleteData; i++) {
							...
							data.referenceImage1 = responseData.data[i].url
							// 对每个元素执行操作
							that.list.push(data)
						}
					}
		...
		generateWorkFlow(prompt, name, audio1, audio2, audio3, seed, totalSteps, width, height, length,
			blocks_to_swap,
			param1, param2, param3, param4, param5) {
			// var newapi = api.replace(/\\/g, "\\\\");
			var that = this

			var jsondata = that.apiFunction(prompt, name, audio1, audio2, audio2, seed, totalSteps, width,
				height,
