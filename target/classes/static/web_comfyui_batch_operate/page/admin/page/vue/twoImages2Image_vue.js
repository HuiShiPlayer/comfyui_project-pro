...
var main = new Vue({
	...
	mounted() {
		var data = localStorage.getItem("twoImages2Image")
		if (data) {
	...
	methods: {
				...
				success: function(res) {
					...
					if(res.status == 200){
						if(res.data.resources && res.data.resources.length>0){
							that.globalReference1 = app.base_path_contact_system.split("admin")[0]+res.data.resources[0]
							that.updateReferenceImage(1)
						}
						if(res.data.resources && res.data.resources.length>1){
							that.globalReference2 = app.base_path_contact_system.split("admin")[0]+res.data.resources[1]
		...
		/* task5.1 */
		checkTask() {
			console.log("checkTask 2I2I")
			var that = this
			var resTaskListStr = localStorage.getItem("resTaskList2i2i");
							...
							if (resCompleteData != null) {
								for (var i = 0; i < resCompleteData.length; i++) {
									...
									if (index !== -1) {
										...
										localStorage.setItem("resTaskList2i2i", resTaskListStr)
									}
		...
		clearCache() {
			...
			localStorage.removeItem('resTaskList2i2i');
			localStorage.removeItem("twoImages2Image")
			location.reload()
		},
		saveDataInCache() {
			console.log('数据缓存')
			...
			localStorage.setItem("twoImages2Image", JSON.stringify(data))
		},
		saveCapcut() {
			var that = this
			...
			try {
				...
				if (that.defaultSource == 1) {
					response = await fetch(app.base_path_contact_system + "/comfyui/rhprompt", {
						...
						body: JSON.stringify({
							"nodeInfoList": that.generateWorkFlow(itemPrompt, s, that
								.totalSteps, w, h, name1, name2, data.param1, data
					...
					response = await fetch(app.base_path_contact_system + "/comfyui/prompt", {
						...
						body: JSON.stringify({
							prompt: that.generateWorkFlow(itemPrompt, s, that.totalSteps, w,
								h, name1, name2, data.param1, data.param2, data.param3,
		...
		generateWorkFlow(prompt, seed, totalSteps, width, height, name1, name2, param1, param2, param3, param4,
			param5) { //todo v2
			// var newapi = api.replace(/\\/g, "\\\\");
			var that = this

			var jsondata = that.apiFunction(prompt, seed, totalSteps, width, height, name1, name2, param1,
			...
			$.ajax({
				...
				data: {
					...
					reference: JSON.stringify({
						referenceImage1: data.referenceImage1,
						referenceImage2: data.referenceImage2,
					})
				},
